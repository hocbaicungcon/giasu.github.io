(async()=>{
 ...
 $('#search').value=params.get('q')||'';$('#grade').value=params.get('grade')||'';$('#type').value=params.get('type')||'';$('#sort').value=params.get('sort')||'new';
 const normalize=s=>s.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/đ/g,'d').toLowerCase();
 function apply(){const q=normalize($('#search').value.trim()),grade=$('#grade').value,type=$('#type').value,sort=$('#sort').value;
 const list=posts.filter(p=>(!category||p.category===category)&&(!tag||p.tags.includes(tag))&&(!grade||String(p.grade)===grade)&&(!type||p.type===type)&&(!q||normalize([p.title,p.description,p.category,...p.tags].join(' ')).includes(q))).sort((a,b)=>sort==='title'?a.title.localeCompare(b.title,'vi'):sort==='old'?a.date.localeCompare(b.date):b.date.localeCompare(a.date));
 ...
 $('#search').addEventListener('input',apply);for(const id of ['grade','type','sort'])$('#'+id).addEventListener('change',apply);
 $('#reset').onclick=()=>{category='';tag='';$('#search').value='';$('#grade').value='';$('#type').value='';$('#sort').value='new';apply();};apply();
})();
