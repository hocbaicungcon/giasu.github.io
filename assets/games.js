...
function startNumbers(){board=Array(16).fill(0);score=0;numberDone=false;spawn();spawn();$('#number-status').textContent='Ghép những ô cùng số nhé!';renderNumbers();}
function move(direction){if(numberDone)return;const result=moveBoard(board,direction);if(!result.changed)return;board=result.board;score+=result.score;spawn();if(board.includes(2048)){numberDone=true;$('#number-status').textContent='Bạn đã đạt 2048! Chúc mừng!';}else if(!canMove(board)){numberDone=true;$('#number-status').textContent='Hết nước đi. Bấm Chơi lại để thử lần nữa.';}else $('#number-status').textContent=result.score?`Cộng ${result.score} điểm.`:'Tiếp tục nào!';renderNumbers();}
...
function wordKey(key){
 if(wordDone)return;
 if(key==='BACKSPACE'){draft=draft.slice(0,-1);renderWords();return;}
 if(/^[A-Z]$/.test(key)){if(draft.length<5){draft+=key;renderWords();}return;}
 if(key!=='ENTER')return;
 if(draft.length!==5){$('#word-status').textContent='Nhập đủ 5 chữ cái trước khi gửi nhé.';return;}
 if(!dictionary.has(draft)){$('#word-status').textContent='Từ chưa có trong bộ từ cơ bản. Thử HOUSE, TRAIN hoặc APPLE. Lượt đoán chưa bị tính.';return;}
 ...
 wordDone=guess===answer||guesses.length===6;draft='';renderWords(true);
...
const starts={numbers:startNumbers,river:startRiver,words:startWords};document.querySelectorAll('[data-restart]').forEach(b=>b.addEventListener('click',()=>starts[b.dataset.restart]?.()));startNumbers();startRiver();startWords();
