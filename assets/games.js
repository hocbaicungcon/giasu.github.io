...
function startNumbers(){numberResult.clear();board=Array(16).fill(0);score=0;numberDone=false;spawn();spawn();$('#number-status').textContent='Ghép những ô cùng số nhé!';renderNumbers();}
function move(direction){if(numberDone)return;const result=moveBoard(board,direction);if(!result.changed)return;board=result.board;score+=result.score;spawn();if(board.includes(2048)){numberDone=true;numberResult.show({won:true,description:'Bạn đã ghép được ô 2048 với '+score+' điểm!'});$('#number-status').textContent='Bạn đã đạt 2048! Chúc mừng!';}else if(!canMove(board)){numberDone=true;numberResult.show({won:false,description:'Bàn đã đầy và không còn nước đi. Bạn đạt '+score+' điểm.'});$('#number-status').textContent='Hết nước đi. Bấm Chơi lại để thử lần nữa.';}else $('#number-status').textContent=result.score?`Cộng ${result.score} điểm.`:'Tiếp tục nào!';if(!numberDone)sound(result.score?'merge':'move');renderNumbers();}
