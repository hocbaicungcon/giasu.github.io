...
if(board){
 ...
 function icon(group,value){
  ...
  if(group==='drink')return svg(`<path d="M9 15h27l-3 25H12Z" fill="${drinks[value]}" stroke="#536f72" stroke-width="2"/><path d="M7 15h31M14 8h18" fill="none" stroke="#536f72" stroke-width="2.3" stroke-linecap="round"/>`);
 ...
 const result=createGameResult(scene,{restart:()=>start(level),next:()=>start(level+1)});
 ...
 function refresh(){
  ...
  for(const slot of slots){const {group}=slot.dataset,house=Number(slot.dataset.house),key=`${group}:${house}`,value=values[group][house],locked=fixed.has(key);slot.replaceChildren();slot.classList.toggle('filled',Boolean(value));slot.classList.toggle('given',locked);slot.setAttribute('aria-label',`${groups[group].label}, nhà ${house+1}`);
   ...
   for(const candidate of options){const button=document.createElement('button'),removed=(excluded[key]||[]).includes(candidate),elsewhere=values[group].some((v,i)=>i!==house&&v===candidate);button.type='button';button.className='einstein-candidate';button.innerHTML=icon(group,candidate);button.classList.toggle('excluded',removed);button.classList.toggle('elsewhere',elsewhere);button.disabled=done||locked||elsewhere;button.title=value?'Bỏ chốt':`${mode==='exclude'?(removed?'Khôi phục':'Loại trừ'):'Chọn'} ${candidate}`;button.setAttribute('aria-label',`${button.title}, ${groups[group].label}, nhà ${house+1}`);button.addEventListener('click',()=>{
   ...
   const remaining=groups[group].options.filter(v=>!(excluded[key]||[]).includes(v)&&!values[group].some((placed,i)=>i!==house&&placed===v));slot.classList.toggle('empty-candidates',!value&&!remaining.length);
  }
  document.getElementById('einstein-progress').textContent=`${Object.values(values).flat().filter(Boolean).length}/25`;
 ...
 function start(next=level){level=Math.max(0,Math.min(einsteinLevels.length-1,next));const entry=einsteinLevels[level];solution=entry.solution;given=entry.given;fixed=new Set(given.map(([group,house])=>`${group}:${house}`));currentClues=entry.clues;clueNodes=currentClues.map(({picture:clue,text})=>{const [a,relation,b,c]=clue,li=document.createElement('li');li.className='einstein-clue';li.title=text;li.setAttribute('aria-label',text);li.classList.toggle('between',(relation==='between'||relation==='sandwich'));const visual=entry=>`<span class="einstein-clue-image">${picture(entry)}</span>`;li.innerHTML=(relation==='between'||relation==='sandwich')?`${visual(b)}<b>${relation==='sandwich'?'⇥':'‹'}</b>${visual(a)}<b>${relation==='sandwich'?'⇥':'‹'}</b>${visual(c)}`:`${visual(a)}<b>${{'<':'‹ ···','>':'··· ›','→':'⇥','←':'⇤','↔':'↔','=':'=','≠':'≠','notAdjacent':'↮','distance2':'↔₂','distance3':'↔₃','edge':'|↔|'}[relation]}</b>${visual(b)}`;return li;});const clueList=document.getElementById('einstein-clue-list');clueList.replaceChildren(...clueNodes);clueList.scrollTop=0;document.getElementById('einstein-level').textContent=`${level+1} / ${einsteinLevels.length}`;levelSelect.value=String(level);document.getElementById('einstein-prev').disabled=level===0;document.getElementById('einstein-next').disabled=level===einsteinLevels.length-1;resetBoard();}
 ...
 document.getElementById('einstein-reset').addEventListener('click',()=>start(level));
