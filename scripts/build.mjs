...
export function build(){
 ...
 for(const file of fs.readdirSync(out,{recursive:true}).filter(file=>file.endsWith('.html'))){
  ...
  html=html.replace('</nav></div></header>',`<div class="display-controls" role="group" aria-label="Tùy chỉnh hiển thị"><button type="button" data-action="less" aria-label="Giảm cỡ chữ" title="Giảm cỡ chữ">A−</button><button type="button" data-action="more" aria-label="Tăng cỡ chữ" title="Tăng cỡ chữ">A+</button><button type="button" data-action="font" aria-label="Chuyển sang chữ có chân" title="Chuyển sang chữ có chân" aria-pressed="false">𝑨</button><button type="button" data-action="theme" aria-label="Đổi giao diện" title="Đổi giao diện" aria-pressed="false">☾</button></div></nav></div></header>`);
