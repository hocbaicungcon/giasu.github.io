...
\begin{baitap}
	...
	\begin{traloi}
		...
		Tập xác định: Điều kiện xác định: $x + 2 \ge 0 \Leftrightarrow  x \ge -2$ nên tập xác định của hàm số là $\mathbb{D} = [-2; +\infty)$.
		
		Đạo hàm: $$y' = \dfrac{1}{2\sqrt{x + 2}} - 1 \quad (\forall x > -2)$$
		
		Tìm điểm tới hạn: Cho $y' = 0$, ta có:$$\dfrac{1}{2\sqrt{x + 2}} - 1 = 0 \iff 2\sqrt{x + 2} = 1 \iff \sqrt{x + 2} = \dfrac{1}{2}$$$$\iff x + 2 = \dfrac{1}{4} \iff x = -\dfrac{7}{4} \quad (\text{thỏa mãn } x \ge -2)$$
		...
		Lập bảng biến thiên: 
			...
			Kết luận:$$\max y = \dfrac{9}{4} \quad \text{tại } x = -\dfrac{7}{4}$$
	\end{traloi}
\end{baitap}
...
\begin{baitap}
	...
	\begin{traloi}
		\dapan{D}
		Giá trị lớn nhất của hàm số là $M = 0$. Dưới đây là các bước giải chi tiết
		\begin{itemize}
...
\begin{baitap}
	...
	\begin{traloi}
		\dapan{C}
		Giá trị lớn nhất của hàm số trên khoảng $(-\infty; +\infty)$ là $M = \sqrt{2}$. Dưới đây là hai cách giải chi tiết:
		\begin{itemize}
...
\begin{baitap}
	...
	\begin{traloi}
		...
		Chúng ta sẽ sử dụng bất đẳng thức $|A| + |B| \ge |A+B|$ nhưng $ f(-1)+f(3) $ chưa triệt tiêu hết $ a,b $ để được một hằng số. Vậy ta cần tìm một điểm nữa. 
		
		Chúng ta sử dụng một tính chất của hàm bậc hai, nếu $ P(x)=x^2+ax+b$ và lấy hai điểm đối xứng quanh \(m\) là $
		m-h$ và $ m+h, $ thì $ P(m-h)+P(m+h)-2P(m)=2h^2. $
		...
		\begin{cases}
			...
			|b + a + 1| = 2
		\end{cases}
		\] và $ b - a + 1, b + 3a + 9, -2b - 2a - 2 $ cùng dấu. Từ đó tìm được \(a = -2\) và \(b = -1\). Lúc đó, ta có hàm số \(g(x) = x^2 - 2x - 1\) có đỉnh là \(x = 1 \in [-1; 3]\). Giá trị lớn nhất của \(f(x)=\left\|g(x)\right\|\) trên đoạn này đúng bằng $ 2 $. 
	\end{traloi}
